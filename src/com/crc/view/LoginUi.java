...
import com.crc.service.impl.UserServiceImpl;

public class LoginUi implements ActionListener{
	private JFrame jf = new JFrame(" 电影购票系统");//窗体
	private Container con = jf.getContentPane();//获得面板
	private ImageIcon bg = new ImageIcon("images/bg.jpg");//图片控件
	private ImageIcon btnbg = new ImageIcon("images/btn.png");//图片控件
	private Toolkit toolkit = Toolkit.getDefaultToolkit();// 获得Toolkit对象
	private Dimension sc = toolkit.getScreenSize();// 获得屏幕尺寸
	
	private JButton in = new JButton("",btnbg);//按钮
	//"进入系统"用背景图
	private JLabel bglabel = new JLabel();//标签
	
	public LoginUi() {//登录界面初始化
		con.setLayout(null);//布局设置为空
		jf.setSize(1000, 600);//设置窗体大小
		jf.setLocation((sc.width - 1000) / 2, (sc.height - 618) / 2);//设置窗体位置
		jf.setResizable(false);// 窗口大小不可变
		jf.setVisible(true);//设置窗体可见
		jf.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//使用 System exit 方法退出应用程序。仅在应用程序中使用。
		con.setVisible(true);//设置窗体可见
		bglabel.setIcon(bg);//设置图标
		bglabel.setBounds(0, 0, 1000, 600);//设置标签位置
		in.setBounds(425, 150, 150, 56);//设置标签位置
		
		con.add(bglabel);//窗体加入标签
		con.add(in);//窗体加入标签
...
class Login implements ActionListener {
	...
	public Login() {
		...
		title.setBounds(400, 10, 370, 30);
		title.setFont(font);
		title.setForeground(Color.black);

		name1.setBounds(350, 140, 85, 30);// 账号的位置大小
		name1.setFont(font1);// 字体
		name1.setForeground(Color.black);// name1字的颜色

		pass1.setBounds(350, 190, 85, 30);// 密码的位置大小
		...
		//验证码
		code1.setBounds(350, 240, 70, 25);
		code1.setFont(font1);
		code1.setForeground(Color.black);
		textCode.setBounds(450, 240, 95, 25);
		textCode.setBorder(null);
		textCode.setFont(font1);
		code2.setBounds(550, 240, 70, 25);
		code2.setFont(font1);
		code2.setText(code());
		code2.setForeground(Color.black);

		//单选框
		choice1.setBounds(370, 290, 80, 25);
		choice1.setSelected(true);// 默认普通用户登录
		choice2.setBounds(500, 290, 80, 25);

		button1.setBounds(380, 340, 90, 25);
